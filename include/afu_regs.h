...
/*
	...
	Example for 180MHz, 7th build on October 31st, 2015: 0x00004650_15103106
 */

#define MMIO_APP_VERSION_REG    0x00000008ull
/*
	-Bits-
	63..40 RO: Release ID (optional): Release Identifier or mkrel release name
		or otherwise designator that uniquely identifies how to
		retrieve the VHDL source code that produced this AFU.
		Higher numbers are later versions.
	39..32 RO: Application Layer Architecture
		0x02: GZIP DDCB with dynamic Huffman support
		0x03: GZIP DDCB with dynamic Huffman support and MMIO driven queue
...
#define MMIO_AFU_CONFIG_REG     0x00000010ull	/* X * 1/200Mhz (set to 512 = 2048 nsec) */
/*
	-Bits-
...
/*
	...
	3...0  RW: Command
		...
		0x2 Stop:  Finish current DDCB, then set accelerator to finished
			(asserting aXh_jdone)
		0x0 NOP
*/

#define MMIO_CASV_REG		0x00000040ull	/* Context is active if bit is set */
/*
...
/*
	Aggravator Register
	===================
	Note: The value that is written into this register will be rotated left every cycle.
...
/*
	...
	20     RW: Use Predefine Huffman Tree
		* 1b = The GZIP Deflate Core Decider Logic is using the
			...
			This tree will be used until the next write or the next power on.
		* 0b = The GZIP Deflate Core Decider Logic is no longer using the
			predefined Huffman tree
	18..16 RW: Predefine Huffman Tree
	12     RW: Enable Decider Window
		* 1b = overrides the GZIP Deflate Core Decider Logic Decider Window value.
	10..0  RW: Maximum Decider Window
		* The Value after power on = 512 (6K).
		* The value of this field multiplied by 12 is the size of the Decider Window.
		* After a write, the Windows remains until the next power on.
		* A larger value could reduce the bandwidth of the GZIP Deflate Core.
		* In case of bad compressible input data, the Decider Window could be smaller 
		as this value. This happens due to GZIP Deflate Core internal buffer sizes. 
...
/*
	...
	** A valid write operation into this register also resets the corresponding
...
/*
	...
	3..0  RW: Command
	Legal commands are:
	0x4 Abort: Stop all DDCB activities for this queue immediately (Argument: Don't care)
	0x2 Stop:  Finish current DDCB, then stop queue (Argument: Don't care)
	0x1 Start: Execute DDCBs (Argument: <Last sequence number to be executed> must be set)
	0x0 NOP
...
/*
	...
	23..8  Non-fatal errors:
		23..22 RO: Reserved
	    	    21 RC: DMA Data Error (see DMA Error Address Register for DMA address
			triggering the error)
		    20 RC: DMA Address Error (see DMA Error Address Register for
			DMA address triggering the error)
		    ...
		    14 RC: Write attempt to DDCB Queue Configuration register with
			first DDCB index > max DDCB index
...
/*
	...
	Address: 0x0000180 + (n+1) * 0x0010000
  	63..0  RO: Counter counting the number of clock cycles during DDCB execution for this context
		(Counter gets reset with every valid DDCBQ CONFIG Register write access;
