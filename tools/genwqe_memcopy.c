...
/**
 * @file	genwqe_memcopy.c
 * @brief	memcopy test utility to try out DDCBs and DMA traffic.
 */
		...
		if (pt->err) {
			...
			if (false == ip.quiet) {
				...
				if (pt->total_usec < 100000) {
					...
					VERBOSE1("%d KiB, in %lld usec, %lu KiB/sec", kib, pt->total_usec, kibs);
				} else {
					total_msec = pt->total_usec / 1000;	/* now msec */
					mibs = (pt->bytes_copied * 1000) / (1024 * 1024) / total_msec;
					VERBOSE1("%d MiB, in %lld msec, %lu MiB/sec", mib, total_msec, mibs);
				}
				VERBOSE1(" %d errors.\n", pt->errors);
			}
		}
	...
	if (false == ip.quiet) {
		...
		if (total_usec) {
			if (total_usec < 100000) {
				...
				VERBOSE0("%d KiB, in %lld usec, %lu KiB/sec", kib, total_usec, kibs);
			} else {
				total_msec = total_usec / 1000;	/* now msec */
				mibs = (bytes_copied * 1000) / (1024 * 1024) / total_msec;
				VERBOSE0("%d MiB, in %lld msec, %lu MiB/sec", mib, total_msec, mibs);
			}
		}
		VERBOSE0(" %d errors.\n", errors);
	}
